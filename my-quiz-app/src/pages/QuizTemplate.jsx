...
import { 
  ...
  RotateCcw, Eye, EyeOff // 누락된 아이콘 추가됨
} from 'lucide-react';
...
export default function QuizTemplate({ quizData, title, subtitle }) {
  ...
  if (!quizData || quizData.length === 0) {
    return <div className="p-10 text-center">데이터를 불러오는 중이거나 데이터가 없습니다.</div>;
  }

  return (
    <div className="min-h-screen bg-gray-100 p-4 font-sans text-gray-800">
      <div className="max-w-3xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden min-h-[600px] flex flex-col">
        {/* Header */}
        <header className="bg-blue-600 text-white p-6 flex justify-between items-center">
          ...
          {mode !== 'landing' && (
            <button 
              ...
              className="text-sm bg-blue-700 hover:bg-blue-800 px-3 py-1 rounded flex items-center gap-1 transition"
            >
          ...
          {mode === 'landing' && (
            <div className="flex flex-col items-center justify-center h-full gap-8 py-10">
              <div className="text-center space-y-2">
                <h2 className="text-xl font-semibold text-gray-700">모드를 선택하세요</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-lg">
                ...
                <button 
                  ...
                  className="flex flex-col items-center p-6 border-2 border-green-100 rounded-xl hover:border-green-500 hover:bg-green-50 transition group"
                >
          ...
          {mode === 'practice' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center text-sm text-gray-500 mb-4">
                ...
                <span className="bg-gray-100 px-2 py-1 rounded">{quizData[currentQuestionIdx].category}</span>
              </div>

              <div className="space-y-4">
                <h3 className="text-xl font-bold leading-relaxed">{quizData[currentQuestionIdx].question}</h3>
                ...
                <div className="space-y-3">
                  {quizData[currentQuestionIdx].options.map((option, idx) => {
                    ...
                    return (
                      <button 
                        key={idx}
                        onClick={() => !showExplanation && handleAnswerSelect(quizData[currentQuestionIdx].id, idx)}
                    ...
                    <div key={q.id} className="border-b pb-6 last:border-0">
                      ...
                      <div className="space-y-2 pl-10">
                        {q.options.map((option, oIdx) => (
                          <label key={oIdx} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-200">
                            <input 
                              type="radio" 
                              name={`q-${q.id}`} 
                ...
                <div className="animate-fadeIn">
                  ...
                  <div className="space-y-6">
                    {quizData.map((q, index) => {
                       ...
                       return (
                         <div key={q.id} className={`p-6 rounded-lg border ${isCorrect ? 'border-gray-200 bg-white opacity-80' : 'border-red-200 bg-red-50'}`}>
                           ...
                           <div className="ml-8 mt-4 bg-white/50 p-3 rounded text-sm text-gray-700">
